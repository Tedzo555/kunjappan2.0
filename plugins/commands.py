...
async def start(client, message):
    if message.chat.type in ['group', 'supergroup']:
        ...
        await asyncio.sleep(2) # 😢 https://github.com/Kunjappan-PSY/psy/blob/master/plugins/p_ttishow.py#L17 😬 wait a bit, before checking.
        ...
        btn = [
            [
                InlineKeyboardButton(
                    "𝙹𝙾𝙸𝙽 𝙼𝚈 𝚄𝙿𝙳𝙰𝚃𝙴 𝙲𝙷𝙰𝙽𝙽𝙴L", url=invite_link.invite_link
                )
            ]
        ]
...
async def gen_link_text(bot, update):
        ...
        photo=random.choice(PICS),
        caption=f"**Message Sharing Link Is Ready** :- https://t.me/share/url?url={quote(message.reply_to_message.text)}", reply_to_message_id=update.id, reply_markup=InlineKeyboardMarkup( [[ InlineKeyboardButton("📤 Share Link 📤", url=f"https://t.me/share/url?url={quote(update.text)}") ]] )       
    )
